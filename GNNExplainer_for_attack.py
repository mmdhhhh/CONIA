...
parser.add_argument('--random_select', type=bool, default=False, help='Random select')
...
if args.model == 'gcn':
    ...
    model = GraphSAGE(nfeat, args.hidden, class_number, args.dropout).to(args.device)
elif args.model == 'gatv2':
    model = GATv2(nfeat, args.hidden_gat, class_number, args.dropout, att_dropout, heads).to(
        args.device)
elif args.model == 'sgc':
    model = SGC(nfeat, args.hidden, class_number, args.dropout, num_layers=2).to(
        args.device)
elif args.model == 'ssgc':
    model = SSGC(nfeat, args.hidden, class_number, args.dropout, num_layers=2).to(
        args.device)
if args.train:
...
if args.device == 'cpu':
    ...
    model.load_state_dict(
        torch.load(os.path.join(save_model_path, args.model + '_' + args.dataset + '_checkpoint.pkl'),
...
for i in range(epo):
    ...
    node_index = torch.from_numpy(n_class_nodes[explain_class])
    ...
    df = df.head(top_k)
    title = f"Class {i} feature importance for top {len(df)} features"
    ax = df.plot(
        kind='barh',
        figsize=(10, 7),
        xlim=[0, float(df['score'].max()) + 5],
        legend=False,
    )
    ...
    plt.show()
    save_path = f'feature_importance/{args.dataset}/'
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    plt.savefig(save_path + f'class {i} feature_importance.png', dpi=150)
